...
9. [Create plots to explore which variables will effect number of citations](#plots)
...
```python 
total_pages = data_set["Ending Page"] - data_set["Beginning Page"]
data_set['total number of pages'] = total_pages
```

# 9. Create plots to explore which variables will effect number of citations <a name="plots"></a>

```{r}
df <- read.csv("TotalCombinedCleaned.csv")

# Plot number of figures vs. citations
plot(df$number.of.figures, df$Total.Citations)

# Plot number of tables vs. citations
plot(df$number.of.tables, df$Total.Citations)

# Plot number of pages vs citations
plot(df$total.number.of.pages, df$Total.Citations, ylim = c(0, 1500))
```
